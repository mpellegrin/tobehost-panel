class mysql::tobehost::config inherits mysql::tobehost {
	...
	define tobehost_mysql_enable($mysql_username, $mysql_password) {
		...
		exec {"mysql-grant-${mysql_username}":
			...
			unless => "/usr/bin/mysql -B -u ${mysql_admin_login} --password='${mysql_admin_password}' -e \"SHOW GRANTS FOR '${mysql_username}'@localhost\" | egrep \"^GRANT ALL PRIVILEGES ON \`${mysql_username}\`.* TO '${mysql_username}'@'localhost'\"",
